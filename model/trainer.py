...
from model.network import VirusHostCoexistenceModel, SimplifiedVirusHostModel
...
class Trainer(object):
    ...
    def train_model(self, virus_data, host_data, coexistence_data, virus_edge_index, host_edge_index,
        ...
        for epoch in range(num_epochs):
            ...
            # output_filtered1 = output[0][1:]
            # output_filtered2 = output[1][1:]
            # output_filtered3 = output[2][1:]
            output_filtered1 = output[0][remaining_indices]
            output_filtered2 = output[1][remaining_indices]
            output_filtered3 = output[2][remaining_indices]
            # output_filtered4 = output[3][:20]
            # target[target == 2] = 1
            target_filterd = target[remaining_indices]
            ...
            # loss = self.loss_fn(output_filtered1, target_filterd)
            # loss = self.loss_fn(output[0], target)
            # loss = self.loss_fn(output_filtered1, target_filterd) + self.loss_fn(output_filtered2, target_filterd)
            loss = loss / 3
            ...
            self.step = epoch
            self.optim.step()
            attention_weights = [output[3], output[4], output[5], output[6]]
        ...
        return (output[0] + output[1]) / 2, attention_weights
    ...
    def create_model(self, input_dim_virus, input_dim_host, hidden_dim=128, transformer_dim=256, output_dim=256,
        ...
        # self.model = SimplifiedVirusHostModel(input_dim_virus, input_dim_host,
        #                                        hidden_dim, transformer_dim, output_dim, pool_size)
        pass
    ...
    def cal_auc(self, ymat, data, fold_number):
        ...
        plt.savefig(self.save_path + 'fpr-precision_xticks.pdf', format='pdf')
    ...
    def cal_auc_80(self, ymat, data, fold_number):
        y_mat_df = pd.DataFrame(ymat)
        y_mat_df.to_csv(self.yamt_result, mode='a', index=False, header=False)
        y_label_df = pd.DataFrame(data)
        y_label_df.to_csv(self.ylabel_result, mode='a', index=False, header=False)
        ...
        plt.title('Receiver Operating Characteristic (ROC) Curve')
        plt.legend(loc='lower right')
        plt.savefig(self.save_path + 'roc_80.pdf', format='pdf')
        plt.close()
        precision, recall, prth = precision_recall_curve(data, ymat)
        aupr = auc(recall, precision)
        ...
        plt.grid()
        # plt.savefig('./ymat_hans_recall-fpr-precision_xticks.pdf', format='pdf')
        plt.savefig(self.save_path + 'fpr-precision_xticks_80.pdf', format='pdf')
        plt.close()
        results = pd.DataFrame({
            'Fold': [fold_number],
            'AUROC': [auroc],
    ...
    def model_runner(self, virus_data, host_data, coexistence_data, virus_edge_index, host_edge_index,
        ...
        for i in range(fold_cv):
            ...
            A0[(i * N // fold_cv):(i + 1) * (N // fold_cv)] = 0
            # A0[:6] = 0.5
            # target_1 = A0
            # for j in range(i * N // fold_cv, (i + 1) * N // fold_cv):
            ...
            resi, attention_weights = self.train_model(virus_data, host_data, target_, virus_edge_index,
                                                       host_edge_index, coexistence_edge_index, target_,
            ...
            attention_weights_all.append(
                [self.to_vertex_weights(attention_weights[0]), self.to_vertex_weights(attention_weights[1]),
                 self.to_vertex_weights(attention_weights[2]), self.to_vertex_weights(attention_weights[3])])
            ...
            auroc_val, aupr_val = self.cal_auc_80(resi[(i + 1 * N // fold_cv):],
                                                  target[(i + 1 * N // fold_cv):], i)
            auroc_val, aupr_val = self.cal_auc(resi[(i * N // fold_cv):(i + 1) * (N // fold_cv)],
                                               target[(i * N // fold_cv):(i + 1) * (N // fold_cv)], i)
        ...
        VH_virus_weights_all = [weights[2] for weights in attention_weights_all]
        VH_host_weights_all = [weights[3] for weights in attention_weights_all]
        # host_weights_final = np.mean(host_weights_all, axis=0)
        # virus_weights_final = np.mean(virus_weights_all, axis=0)
        # VH_virus_weights_final = np.mean(VH_virus_weights_all, axis=0)
        # VH_host_weights_final = np.mean(VH_host_weights_all, axis=0)
        host_weights_final = host_weights_all[-1]
        virus_weights_final = virus_weights_all[-1]
        VH_virus_weights_final = VH_virus_weights_all[-1]
        VH_host_weights_final = VH_host_weights_all[-1]
        ...
        pd.DataFrame(VH_virus_weights_final).to_csv(self.save_path + 'VH_host_weights_final.csv', index=False)
        pd.DataFrame(VH_host_weights_final).to_csv(self.save_path + 'VH_virus_weights_final.csv', index=False)
        import matplotlib.pyplot as plt
        ...
        sns.heatmap(VH_virus_weights_final, cmap='viridis')
        plt.title('VH Virus Weights Heatmap')
        plt.savefig(self.save_path + 'VH_virus_weights_heatmap.png')
        plt.close()
        sns.heatmap(VH_host_weights_final, cmap='viridis')
        plt.title('VH Host Weights Heatmap')
        plt.savefig(self.save_path + 'VH_host_weights_heatmap.png')
        plt.close()
        # if self._config.with_cuda:
