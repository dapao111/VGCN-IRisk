...
class VAE_transmission_model(nn.Module):
    def __init__(self, input_dim_virus, input_dim_host, hidden_dim, max_frequence, embed_dim, dropout_rate=0.5):
        ...
        # self.embedding_host = nn.Embedding(max_frequence[1], embed_dim)  # 4个字母，嵌入到embed_dim维
        ...
        self.virus_dim = [embed_dim, 512,256,hidden_dim, 64]
        ...
        self.decoder_dim = [64 * 2 + 10, 256, 512,1024]
    ...
    def forward(self, virus_data, host_data, noise=None):
        ...
        # host_data = self.embedding_host(host_data)
        # virus_data = x.mean(1)
        # host_data = host_data.mean(1)
        ...
        # virus_data_mean = self.transformer(virus_data_mean)  # (seq_len, batch_size, embed_dim)
    ...
    def save_pretrained(self, file_path,step):
        ...
        # 保存模型权重
        model_weights_path = os.path.join(file_path, 'pytorch_model'+str(step)+'.bin')
